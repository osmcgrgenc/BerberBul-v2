...
export async function GET() {
  ...
  const { data: appointments, error } = await supabase
    ...
    .select(`
      *,
      customer:customer_id(id, business_name, email, phone_number), // Include customer email and phone_number
...
export async function PUT(request: Request, { params }: { params: { appointmentId: string } }) {
  ...
  if (!appointmentId || !status) {
    return NextResponse.json({ error: 'Randevu ID ve durum zorunludur.' }, { status: 400 });
  }
  ...
  try {
    ...
    if (fetchError || !appointment) {
      return NextResponse.json({ error: 'Randevu bulunamadı veya yetkiniz yok.' }, { status: 404 });
    }
